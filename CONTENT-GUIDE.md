...
- `Float`의 `01`~`05` 이미지는 PC 홈 장식 이미지로만 사용되며 프로젝트 링크가 없습니다. 모바일에서는 표시되지 않습니다.
- 프로젝트 폴더의 첫 번째 이미지가 대표 이미지가 됩니다.
- 지원 형식: JPG, JPEG, PNG, WebP, TIFF, AVIF, GIF. 움직이는 GIF는 애니메이션을 유지한 WebP로 자동 변환됩니다.
...
클라이언트: ROADIC
참고 링크: https://example.com/project
```
...
`참고 링크`를 입력하면 프로젝트 정보 영역에 `Visit project ↗` 링크가 생기고 새 창에서 해당 라이브 페이지가 열립니다.
