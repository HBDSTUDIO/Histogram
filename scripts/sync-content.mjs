...
async function main() {
  ...
  for (const sectionFolder of sectionFolders) {
    ...
    for (const projectFolder of await directories(contentsRoot)) {
      ...
      const sourceImages = entries.filter((entry) => entry.isFile() && imageExtensions.has(path.extname(entry.name).toLowerCase())).map((entry) => entry.name).sort(collator.compare);
      ...
      for (const [index, imageName] of sourceImages.entries()) {
        ...
        await sharp(path.join(projectRootPath, imageName)).rotate().resize({ width: 2560, withoutEnlargement: true }).webp({ quality: 82, effort: 4 }).toFile(outputPath);
  ...
  await fs.writeFile(dataFile, `${JSON.stringify({ siteSettings, sections, projects }, null, 2)}\n`, "utf8");
  console.log(`완료: 프로젝트 ${projects.length}개, 웹 이미지 ${projects.reduce((sum, project) => sum + project.images.length, 0)}개`);
}
