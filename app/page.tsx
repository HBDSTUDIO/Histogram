...
function FloatingImage({ image, className }: { image: (typeof floatImages)[number]; className: string }) {
  return <div className={`floating-card ${className}`}><Image src={image.src} alt={image.alt} fill sizes="22vw" className="floating-image" /></div>;
}

function ProjectCard({ project }: { project: Project }) {
  return <Link className="index-card" href={`/project/${project.slug}`}>
    <div className="index-image-wrap"><Image src={project.cover.src} alt={project.cover.alt} fill sizes="(max-width: 767px) 100vw, 33vw" className="index-image" unoptimized={project.cover.animated} /></div>
