...
function PhotoWallRows({ rows }: { rows: PortfolioImage[][] }) {
  return <>{rows.map((row, rowIndex) => <div className="photo-wall-row" key={rowIndex}>{row.map((image) => <figure key={image.src} style={{ "--image-ratio": image.width / image.height } as CSSProperties}><Image src={image.src} alt={image.alt} width={image.width} height={image.height} sizes="(max-width: 700px) 60vw, 35vw" /></figure>)}</div>)}</>;
}
